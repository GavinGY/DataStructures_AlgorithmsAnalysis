...
gavin@Dream:~/project/createnfimg$ ./createnfimg 

USAGE:
  createnfimg <options>

DESCRIPTION:
  createnfimg takes an input file, generates the ecc bytes for it and creates an output
  file with both the data and the OOB bytes.

OPTIONS:
    -b <bch_level>     -- Level of BCH correction; default=4.
    -c <partial_page_size>   -- Partial page size in bytes; default=512.
    -d <pad_amount>    -- OOB padding amount to fill after ECC is finished,
                          for example with newer NAND devices that have 128 bytes OOB per page,
                          treat part like OOB=64 (-r 16) and set this value to 64
                          to pad 64 bytes after the OOB 64 bytes; default=0.
    -h                 -- Prints help text.
    -i <infile>        -- Name of the input file.  Output files will be 'infile.out'.
    -j <0|1>           -- 1=Add JFFS2 cleanmarker to spare area: default=1.
    -l <0|1>           -- 1=Select little endian clean marker (default is big endian)
    -m <field_order>   -- Order of the finite field for BCH; default=13.
    -n <pages_per_block> -- Pages per erase block; default=64.
    -p <page_size>     -- Page size in bytes; default=2048.
    -r <oob_size>      -- OOB size in bytes per 512 byte subpage; default=16.
    -s <seed>          -- Random seed value; default=1.
    -t <0|1>           -- 1=Trim input file to a page boundary (eliminate extraneous signature bytes); default=1.
...
int createnfimg(int argc, char **argv)
{
    ...
    */
    ...
    //strcat(p_outfile_out_name, ".out"); //Gavin modify
	strcat(p_outfile_out_name, ".oob");   //Gavin add
